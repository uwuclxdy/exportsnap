...
///
/// Scope: this covers the two arms where one panel owns the whole body. The stacked arm splits it,
/// so what keeps that one safe is its own `area.height >= stack_height` gate, not this constant.
const GUARANTEED_INTERIOR_ROWS: u16 = shell::COMPACT_HEIGHT - shell::HEADER_ROWS - shell::FOOTER_ROWS - widgets::BORDER_ROWS;
...
/// noise, and the contract's own storage examples are all abbreviated. Counts on this screen still
/// take the full comma-separated form, where the exact number is the information.
...
fn head_ellipsis(text: &str, budget: usize) -> String {
