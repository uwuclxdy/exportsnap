...
fn the_form_renders_every_row_with_value_and_provenance() {
    ...
    assert_eq!(row(buffer, 1), panel_row(&format!("{}   · file", padded("❯ output dir  /file/out", 15)), 29));
    assert_eq!(row(buffer, 2), panel_row("  theme  full  compatible   · file", 42));
    assert_eq!(row(buffer, 3), panel_row(&format!("{}   · detection", padded("  ffmpeg path  /detected/ffmpeg", 8)), 23));
    ...
    assert_eq!(buffer[(43, 1)].style().fg, Some(palette.text_faint));
...
fn the_form_renders_at_57_columns_with_the_clause_gap() {
    ...
    assert_eq!(row(buffer, 1), panel_row(&format!("{}   · file", padded("❯ output dir  /file/out", 15)), 6));
...
fn an_empty_draft_ellipsises_the_effective_value_and_marks_it_as_placeholder() {
    ...
    // attribute). The trailing `…` is that marker, at the slot's last cell.
    ...
    let terminal = render_80(&settings);
    let buffer = terminal.backend().buffer();
    // The 32-cell default head-ellipsises to 23 cells, then the marker: the slot holds
    // `…ng/tail/exportsnap-out…`, and the clause keeps its column three cells later.
    assert_eq!(
        row(buffer, 1),
        panel_row("✎ output dir  …ng/tail/exportsnap-out…   · default", 26),
        "the ghost ellipsises to the slot and its marker stays inside it"
    );
    assert_eq!(buffer[(39, 1)].symbol(), "…", "the marker is the slot's last cell");
    assert_eq!(buffer[(43, 1)].symbol(), "·", "the clause keeps its column, three cells past the slot");
}
...
fn a_flag_overridden_row_shows_the_flag_value_and_reads_as_overridden() {
    ...
    let terminal = render_80(&settings);
    let buffer = terminal.backend().buffer();
    assert_eq!(row(buffer, 1), panel_row(&format!("{}   · flag", padded("❯ output dir  /flag/out", 15)), 29));
...
fn a_commit_writes_the_file_and_a_restart_reads_it_back() {
    ...
    let expected = panel_row(&format!("{}   · file", padded("❯ output dir  /new", 20)), 29);
...
fn an_empty_commit_drops_the_key_and_the_row_shows_the_default() {
    ...
    let expected = panel_row(&format!("{}   · default", padded(&shown, 2)), 26);
...
fn a_wide_char_draft_places_the_cursor_and_tag_in_display_cells() {
    ...
    assert_eq!(row(buffer, 1), panel_row(&format!("✎ output dir  中 A{}   · default", " ".repeat(21)), 26));
    ...
    let terminal = render_80(&settings);
    let buffer = terminal.backend().buffer();
    assert_eq!(terminal.backend().cursor_position(), Position::new(38, 1), "the caret stays at the last cell of the slid window");
    assert_eq!(buffer[(43, 1)].symbol(), "·", "the provenance clause stays inside the interior");
    // The window holds 11 wide chars, not 23 — a char-bounded window would spill 46 cells
    // and push the clause past the panel edge.
    assert_eq!(row(buffer, 1), panel_row(&format!("✎ output dir  {}{}   · default", "中 ".repeat(11), "  "), 26));

    // A mid-draft caret is where the window's cell cap binds: 26 cells of wide text after
    // the cut would spill 52 cells, so the window must hold 12 wide chars — the slot, full —
    // and the clause stays at its column.
    let mut settings = settings_in(&dir);
    settings.handle_key(key(KeyCode::Enter));
    for _ in 0..40 {
        settings.handle_key(key(KeyCode::Char('中')));
    ...
    assert_eq!(buffer[(43, 1)].symbol(), "·", "the clause stays put with wide chars still in the draft");
    assert_eq!(row(buffer, 1), panel_row(&format!("✎ output dir  {}   · default", "中 ".repeat(12)), 26));
}
